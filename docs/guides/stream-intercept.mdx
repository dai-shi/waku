...
In some cases, you might want to inject additional content into the SSR stream before it is sent
to the client. This can be useful for injecting propperties onto the document root element for instanceof,
like the `lang` attribute or a class name based on request details.

### Step 1: Create a custom middleware for Waku

Create a new file in your project, for example `src/middleware/streamInterceptor.ts`:
...
In your `waku.config.ts` file, import the middleware and add it to the `middlewares` array:

```ts
import streamInterceptor from "./src/middleware/streamInterceptor";
...
After adding the middleware, you will need to restart the server for the changes to take effect.
(If you had a server running in the background)

### Step 4: Verify the changes

After restarting the server, you can verify that the changes have taken effect by inspecting the
HTML source of your page. You should see the `lang` attribute and the `class` attribute on the
`<html>` element.
