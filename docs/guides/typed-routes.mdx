...
To read, update, or navigate with search on the client, the codec has to be available there. Wrap your app with `SearchCodecsProvider_UNSTABLE`. Because codecs hold functions, the provider must be rendered from a `'use client'` module that imports them:
