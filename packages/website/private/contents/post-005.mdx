...
## Root elements
There are many cases where you may want to  customize the attributes of the root `<html>`, `<head>`, or `<body>` elements. You can now do so using the optional root element API. It can be used in both Waku’s file-based “pages router” and the underlying low-level API for programmatic routing.
...
### Low-level API
There is also a new `createRoot` function exposed from `createPages`.
